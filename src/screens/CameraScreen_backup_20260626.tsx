...
// CameraScreen v2.5 (2026-06-25)
// 갤러리 자동 저장 추가
// - autoCapture 시 Filesystem.writeFile → Documents 저장
// - PROCESS_WIDTH/HEIGHT 160x120
// - willReadFrequently: true
// - QC_MIN_BLUR_VARIANCE 20
...
const FALLBACK_FAIL_COUNT   = 10
const QC_MIN_BRIGHTNESS     = 40
const QC_MAX_BRIGHTNESS     = 220
const QC_MIN_BLUR_VARIANCE  = 20
...
const CameraScreen = ({
  ...
  const runPhysicalVerify = useCallback(async (roiDataUrl: string): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        if (failCountRef.current >= FALLBACK_FAIL_COUNT) {
  ...
  const autoCapture = useCallback(async (candidate: CardCandidate) => {
    ...
    try {
      ...
      const fileName = `geocam_${Date.now()}.png`
  ...
  const startDetectLoop = useCallback(() => {
    ...
    const loop = (timestamp: number) => {
      ...
      if (timestamp - lastFrameTimeRef.current < DETECT_INTERVAL_MS) {
      ...
      if (!video || !canvas || !cameraReady || capturing || isCapturingRef.current) {
        rafIdRef.current = requestAnimationFrame(loop)
        return
      }

      const ctx = canvas.getContext('2d', { willReadFrequently: true })
      if (!ctx) { rafIdRef.current = requestAnimationFrame(loop); return }
      ...
      if (!candidate) {
        ...
        setDetectState('idle')
        setStableProgress(0)
        setGuideBox(prev => ({ ...prev, visible: false }))
        rafIdRef.current = requestAnimationFrame(loop)
        return
      }
