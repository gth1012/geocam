...
const LoginScreen = ({ safeGoHome, onLoginSuccess }: LoginScreenProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!data.success) {
        setError(data.error?.message || t('error.server'))
        return
      }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px' }}>
        {mode === 'signup' && (
          <input type="text" placeholder={t('login.nicknamePlaceholder')} value={nickname} onChange={e => setNickname(e.target.value)} style={inputStyle} />
        )}
        <input type="email" placeholder={t('login.emailPlaceholder')} value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} />
        <input type="password" placeholder={t('login.passwordPlaceholder')} value={password} onChange={e => setPassword(e.target.value)} style={inputStyle} />
      </div>
      ...
      >
        {loading ? t('common.loading') : mode === 'login' ? t('login.title') : t('login.signupTitle')}
      </button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        ...
        <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '11px' }}>{t('login.continueWith')}</span>
        <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.06)' }} />
      </div>
