  ...
  useEffect(() => {
    const fetchCollection = async () => {
      ...
      try {
        ...
        const res = await fetch(`${NEO_API_BASE}/ownership/${deviceId}`)
        if (!res.ok) {
        ...
        {sellSuccess ? (
          ...
          <>
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px', marginBottom: '24px' }}>
              <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '11px', marginBottom: '4px' }}>{getAgencyName()}</p>
              <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '15px', fontWeight: '500', marginBottom: '4px' }}>{getDisplayName(selectedItem)}</p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>{getArtistName()}</p>
            </div>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#0a0a0c', padding: '20px', paddingTop: 'max(48px, env(safe-area-inset-top))' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px', marginBottom: '12px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={valueStyle}>{getAgencyName()}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={labelStyle}>{t('collection.series')}</span>
            <span style={valueStyle}>{getDisplayName(selectedItem)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={labelStyle}>{t('collection.artist')}</span>
            <span style={valueStyle}>{getArtistName()}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={labelStyle}>{t('collection.status')}</span>
            <span style={{ color: '#4ade80', fontSize: '14px', fontWeight: '500' }}>{selectedItem.ownership_status}</span>
          </div>
        </div>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#0a0a0c', padding: '20px', paddingTop: 'max(48px, env(safe-area-inset-top))' }}>
      ...
      {!loading && error && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px' }}>
          ...
          <button
            ...
            style={{ padding: '10px 20px', borderRadius: '10px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.6)', fontSize: '13px', cursor: 'pointer' }}
          >
            다시 시도
          </button>
        </div>
      ...
      {!loading && !error && items.length === 0 && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', padding: '0 20px' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '14px', textAlign: 'center', lineHeight: '1.6' }}>
            아직 등록된 컬렉션이 없습니다.<br />
            <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.2)' }}>
              GeoCapsule을 열고 ORIGINAL 검증을<br />완료하면 My Collection에 저장됩니다.
            </span>
          </p>
        </div>
      )}
          ...
          {items.map(item => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '11px', marginBottom: '2px' }}>{getAgencyName()}</p>
                <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '14px', fontWeight: '500', marginBottom: '3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {getDisplayName(item)}
                </p>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px', marginBottom: '6px' }}>{getArtistName()}</p>
                <span style={{ color: '#4ade80', fontSize: '11px', background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: '4px', padding: '2px 6px' }}>
